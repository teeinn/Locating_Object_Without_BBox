...
def build_dataset_personal(directory,
                  transforms=None,
                  max_dataset_size=float('inf'),
                  ignore_gt=False,
                  seed=0):
    ...
    dset = CSVDataset_personal(directory=directory,
                      transforms=transforms,
                      max_dataset_size=max_dataset_size,
                      ignore_gt=ignore_gt,
                      seed=seed)
...
class CSVDataset_personal(torch.utils.data.Dataset):
    def __init__(self,
                 directory,
                 transforms=None,
                 max_dataset_size=float('inf'),
    ...
    def __getitem__(self, idx):
        ...
        filename = self.root_dir.split('/')[-1]
        dictionary = {'filename': filename}
        img = Image.open(self.root_dir)

        # Record original size
        orig_width, orig_height = get_image_size.get_image_size(self.root_dir)
        with torch.no_grad():
            orig_height = torch.tensor(orig_height,
                                       dtype=torch.get_default_dtype())
            orig_width = torch.tensor(orig_width,
