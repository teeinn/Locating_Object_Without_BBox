...
def threshold(array, tau, nblocks):
    """
    ...
    if tau == -1:
        # Otsu thresholding
        # test_array_gpu = cuda.mem_alloc(array.nbytes)
        # cuda.memcpy_htod(test_array_gpu, array)
        ...
        # test_array_doubled = np.empty_like(array)
        # cuda.memcpy_dtoh(test_array_doubled, test_array_gpu)
        #
        # array_scaled = test_array_doubled.round().astype(np.uint8).squeeze()
...
def paint_circles(img, points, color='iceblue', crosshair=False):
    """
...
def to_normal_image(img, color='red'):
    """
    Paint points as circles on top of an image.
