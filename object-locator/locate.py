...
print("set the module:{}".format(end-start))
...
def calculate_distance_of_two_points_of_boxes(first_point, second_point):
    ...
    [xc1, yc1, h1] = first_point[1], first_point[0], 75
    [xc2, yc2, h2] = second_point[1], second_point[0], 75
...
def draw_line_between_points(point_a, point_b, img, img_path, total_cnt):
    y, x = point_a[0], point_a[1]
    y2, x2 = point_b[0], point_b[1]

    test_img = cv2.line(img, (x,y), (x2,y2), (0,0,255), thickness=2, lineType=8)
    test_img = cv2.putText(test_img, "2m violation", (3, 30), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255),
                           thickness=3)
    test_img = cv2.putText(test_img, "cnt:{}".format(total_cnt), (3,70), cv2.FONT_HERSHEY_PLAIN, 2, (0,0,255), thickness=3)

    cv2.imwrite(img_path,
                test_img)
    return test_img

def find_short_distance_between_objects(centroids, img, img_path):

    img = img.transpose((1, 2, 0))[:, :, ::-1]
    total_cnt = len(centroids)

    for idx, point in enumerate(centroids):
        for i in range(0, len(centroids)):
            if idx != i:
                another_point = centroids[i]
                # distance = math.hypot(point[1]-another_point[1], point[0]-another_point[0])
                distance = calculate_distance_of_two_points_of_boxes(point, another_point)

                if distance <= 200:
                    img = draw_line_between_points(point, another_point, img, img_path, total_cnt)
...
for batch_idx, (imgs, dictionaries) in tqdm(enumerate(testset_loader),
    ...
    print("img to device:{}".format(end - start))
    ...
    print("Pull info from this batch and move to device:{}".format(end - start))
    ...
    print("Lists->tensor batches:{}".format(end - start))
    ...
    print("tensor->float&numpy:{}".format(end - start))
    ...
    print("feed forward:{}".format(end - start))
    ...
    print("Convert to original size:{}".format(end - start))
    ...
    print("Overlay output on original image as a heatmap:{}".format(end - start))
    ...
    print(" Tensor -> int:{}".format(end - start))
    ...
    for t, tau in enumerate(args.taus):
        ...
        if args.paint:
            ...
            os.makedirs(os.path.join(args.out,
                                     'intermediate',
                                     'distance_on_estimated_map',
                                     f'tau={round(tau, 4)}'), exist_ok=True)
            find_short_distance_between_objects(centroids_wrt_orig, img_with_x_n_map, os.path.join(args.out,
                                     'intermediate',
                                     'distance_on_estimated_map',
                                     f'tau={round(tau, 4)}',
                                     dictionaries[0]['filename']))
        ...
        print("find_distance:{}".format(end - start))
        ...
        print("args.evaluate:{}".format(end - start))
        ...
        print("Save a new line in the CSV corresonding to the resuls of this img:{}".format(end - start))
